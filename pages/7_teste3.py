...
def carregar_e_processar_excel(uploaded_file):
    ...
    # Calcular a média de consumo de ração para cada dia
    medias_racao_ct = df_racao_ct.iloc[:, 2:].mean()
    medias_racao_dt = df_racao_dt.iloc[:, 2:].mean()

    return medias_peso, erro_padrao_peso, medias_racao_ct, medias_racao_dt
...
if uploaded_file is not None:
    ...
    medias_peso, erro_padrao_peso, medias_racao_ct, medias_racao_dt = carregar_e_processar_excel(uploaded_file)
    ...
    # Plotar gráfico de consumo de ração
    fig_racao, ax_racao = plt.subplots(figsize=(10, 6))
    ax_racao.plot(medias_racao_ct.index, medias_racao_ct.values, label="Caixa CT", marker='o', color='g')
    ax_racao.plot(medias_racao_dt.index, medias_racao_dt.values, label="Caixa DT", marker='o', color='r')
    ...
    ax_racao.set_title("Consumo Médio de Ração por Caixa")
    ax_racao.legend()
