...
def carregar_e_processar_excel(uploaded_file):
    ...
    for classe in df_peso["Classe do Animal"].unique():
        ...
        medias_peso[classe] = df_classe.iloc[:, 2:].mean()
        erro_padrao_peso[classe] = df_classe.iloc[:, 2:].std() / np.sqrt(df_classe.shape[0])
    ...
    for classe in df_racao["Classe da Caixa"].unique():
        medias_racao[classe] = df_racao[df_racao["Classe da Caixa"] == classe].iloc[:, 2:].mean()
...
def plotar_pesagem(medias_peso, erro_padrao_peso):
    dias = np.arange(1, len(next(iter(medias_peso.values()))) + 1)  # Define os dias com base na primeira série de dados
    ...
    fig.update_layout(title="Média de Peso dos Animais por Classe", xaxis_title="Dias", yaxis_title="Peso (g)")
    st.plotly_chart(fig)

# Função para plotar o consumo de ração
def plotar_consumo_racao(medias_racao):
    dias = np.arange(1, len(next(iter(medias_racao.values()))) + 1)
    ...
    fig.update_layout(title="Consumo Médio de Ração por Classe", xaxis_title="Dias", yaxis_title="Consumo (g)")
    st.plotly_chart(fig)
...
if uploaded_file is not None:
    ...
    plotar_pesagem(dados["medias_peso"], dados["erro_padrao_peso"])

    # Plota o gráfico de consumo de ração para todas as classes
    plotar_consumo_racao(dados["medias_racao"])
