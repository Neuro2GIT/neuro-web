...
class ResultadoAnimal:
    ...
    def exibir_resultados(self):
        st.write(f"### Resultados para: {self.nome_planilha}")
        st.dataframe(self.df[["ID", "Classe do animal", "Tempo no objeto novo", 
                              "Tempo no objeto familiar", "Índice de discriminação", 
                              "Índice de preferência", "Discriminação absoluta"]])

    def salvar_excel(self, writer):
        self.df.to_excel(writer, sheet_name=self.nome_planilha, index=False)
